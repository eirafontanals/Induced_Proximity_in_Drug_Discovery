...
png("ubiquitination_crbn.png",
    width = 2400, height = 1500, res = 180, bg = "transparent")
grid.newpage()

C <- list(
  e1_fill="#A8D96B", e1_brd="#5A8A1E",
  e2_fill="#68C7D4", e2_brd="#267986",
  e3_fill="#6B8FD4", e3_brd="#2E5BAA",
  ub_fill="#E8A838", ub_brd="#A06010", ub_txt="#3A2000",
  tgt_fill="#945057", tgt_brd="#542C30",
  pro_fill="#9B7FD4", pro_brd="#5B3FA0",
  pro_ring="#C4B0E8",
  cul_fill = "#7B52B0", cul_brd = "#3A2080",
  rbx_fill = "#C0383A", rbx_brd = "#7A1012",
  ddb_fill = "#63605E", ddb_brd = "#403D3C",
  crbn_fill="#E0B874", crbn_brd="#574423",
  zoom_brd="#7AAFD4",
  arrow="#333333",
  txt_main="#1A1A1A",
  txt_sub="#555555",
...
lbl(X3+0.008, Y+0.115, "Protein", size=12, col="#DDBBBB")
...
arr(X3+0.092, Y+0.160, X3+0.068, Y+0.137, col="#666666", lwd=1.8, lty=2)
...
# STEP 5 – Proteasome (bigger, moved right)
X5 <- 0.895
RX  <- 0.050   # wider rings
RH  <- 0.034   # taller rings
...
# peptide squiggles — shifted right so they clear the barrel
...
# Arrow from step 4 → proteasome (shifted right to avoid Ub chain)
arr(X4+0.10/2+0.022, Y, X5-RX-0.006, Y)

# ── ZOOM BOX ─────────────────────────────────────────────
ZX <- 0.50; ZY <- 0.23; ZW <- 0.36; ZH <- 0.33
ZOOM_TOP <- ZY + ZH/2

# CHANGED: arrow starts below the subtitle "E3 transfers Ub to target" (YS)
ARROW_START_Y <- YS - 0.018
# CHANGED: "zoom in" label centered on the arrow's midpoint
...
# CHANGED: CRL4^CRBN label – bottom-left inside zoom box
lbl(ZX - ZW/2 + 0.030, ZY - ZH/2 + 0.022,
    "CRL4", size=14, face="bold", col="#7AAFD4", just="left")
# superscript "CRBN" — placed slightly right and up from "CRL4"
lbl(ZX - ZW/2 + 0.077, ZY - ZH/2 + 0.032,
    "CRBN", size=10, face="bold.italic", col="#7AAFD4", just="left")
...
lbl(ZX, CUL_Y, "CUL4A", size=16, face="bold", col="white")
...
# ── DDB1 – right side of CUL4A ────────────────────────
DDB_X <- ZX + 0.058
DDB_Y <- ZY + 0.028
rr(DDB_X, DDB_Y, 0.074, 0.054, C$ddb_fill, C$ddb_brd, lwd=2, r=0.01)
lbl(DDB_X, DDB_Y, "DDB1", size=14, face="bold", col="white")
seg(DDB_X, CUL_Y - CUL_H/2,
    DDB_X, DDB_Y + 0.028,
    col=C$cul_brd, lwd=2.5)
...
CRBN_X <- DDB_X
CRBN_Y <- ZY - 0.058
rr(CRBN_X, CRBN_Y, 0.092, 0.054, C$crbn_fill, C$crbn_brd, lwd=2.5, r=0.01)
lbl(CRBN_X, CRBN_Y, "CRBN", size=16, face="bold", col="#3A2800")
seg(CRBN_X, DDB_Y - 0.028,
    CRBN_X, CRBN_Y + 0.028,
    col=C$ddb_brd, lwd=2)

# ── Target bound to CRBN ──────────────────────────────
TGT_X <- CRBN_X
TGT_Y <- ZY - 0.128
rr(TGT_X, TGT_Y, 0.082, 0.044, C$tgt_fill, C$tgt_brd, lwd=2, r=0.015)
lbl(TGT_X, TGT_Y, "Target", size=13, face="bold", col="white")
seg(TGT_X, CRBN_Y - 0.028,
    TGT_X, TGT_Y + 0.023,
    col=C$crbn_brd, lwd=2)

dev.off()
cat("Done.\n")
knitr::include_graphics("ubiquitination_crbn.png")
```
